...
class SelfAttention(nn.Module):
    ...
    def forward(self, input):
        ...
        # batch, channel, height = input.shape
        n_head = self.n_head
        ...
        # qkv = self.qkv(norm).view(batch, n_head, head_dim * 3, height)
        ...
        # attn = attn.view(batch, n_head, height, height)

        out = torch.einsum("bnhwyx, bncyx -> bnchw", attn, value).contiguous()

        out = self.out(out.view(batch, channel, height, width))
        # out = self.out(out.view(batch, channel, height))
